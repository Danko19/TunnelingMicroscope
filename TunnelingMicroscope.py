...
def prepareGraph(methodName):
    ...
    minor_ticks = np.arange(Model.X_MIN, Model.X_MAX + 15 + 1, 1)
    ...
    plt.ylim(Model.Y_MIN, Model.Y_MAX + 15 + 1)
...
for u in U_d:    
    prepareGraph("Профилограмма (u={0})".format(u))
    for z0 in Z_d:
        results = []
        ...
        plt.plot(xs, zs, label = "z0={0}".format(z0))
        if z0 == 5:
            plt.plot(xs, ms)
    plt.legend()
    plt.savefig("D:\\profs\\Профилограмма u={0}.png".format(u))
    plt.show()